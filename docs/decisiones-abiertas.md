...
El fundador quiere Coins comprables y retirables por dinero real, bajo la tesis "habilidad, no azar". **Estado: NO implementado como rail de pagos y condicionado a habilitación legal.** Razones registradas: (1) la decisión original documentó el riesgo Conajzar/procesadores; (2) el motor de penales tiene azar por diseño (15% de errado) — debilita la tesis de skill puro ante un regulador; (3) faltan licencia/dictamen, KYC/AML, verificación de edad (D5 abierta) y procesador que acepte el rubro. **Prerequisitos para reabrir:** dictamen legal escrito + licencia o resolución del regulador + KYC + verificación de edad + partner de pagos. El ledger ya queda extensible por `kind` para enchufar una capa de compra si eso llega.

**Implementado 2026-07-05 (mock, sin rail de pagos):** el botón `+` de la pill de coins abre un modal que arma un mensaje de WhatsApp al admin (`+595972137968`, "Hola! quiero comprar J {cantidad} coins", ratio 1:1 Gs). **No acredita nada**: la acreditación es manual por base de datos. Es una solicitud fuera de banda, no un procesamiento de pago. No cambia el estado legal de arriba.

## D9 — Tienda: canjes por premios reales (pedido del fundador, 2026-07-05)
El modelo de negocio suma canje de Coins por premios reales (PS5, juegos, ropa de auspiciantes). **Estado: solo mock.** La sección "Canjes" de la Tienda muestra cards "Próximamente" sin lógica. El sistema real de canjes (catálogo, stock, logística con auspiciantes, verificación) es una fase futura dedicada. Igual que D8, canjear valor real por una moneda ganada jugando toca terreno regulatorio: definir con asesoría legal antes de habilitarlo.
